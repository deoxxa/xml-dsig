...
signatures["http://www.w3.org/2001/04/xmldsig-more#ecdsa-sha256"] = {
  sign: function sign(options, data) {
    return crypto.createSign("ECDSA-SHA256").update(data).sign(options.privateKey, "base64");
  },
  verify: function verify(options, data, signature) {
    return crypto.createVerify("ECDSA-SHA256").update(data).verify(options.publicKey, signature, "base64");
  },
  name: function name() {
    return "http://www.w3.org/2001/04/xmldsig-more#rsa-sha256";
  },
...
signatures["http://www.w3.org/2000/09/xmldsig#dsa-sha1"] = {
  sign: function sign(options, data) {
    return crypto.createSign("DSA-SHA1").update(data).sign(options.privateKey, "base64");
  },
  verify: function verify(options, data, signature) {
    return crypto.createVerify("DSA-SHA1").update(data).verify(options.publicKey, signature, "base64");
  },
  name: function name() {
