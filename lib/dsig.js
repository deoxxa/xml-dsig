...
dsig.verifySignature = function verifySignature(_node, _signatureElement, _options) {
  ...
  for (var i=0;i<referenceElements.length;++i) {
    ...
    for (var j=0;j<transformElements.length;++j) {
      var inclusiveNamespaces = [],
          inclusiveNamespaceElements =
            transformElements[j].getElementsByTagNameNS("http://www.w3.org/2001/10/xml-exc-c14n#", "InclusiveNamespaces");

      for (var k = 0; k < inclusiveNamespaceElements.length; ++k) {
        if (inclusiveNamespaceElements[k].hasAttribute("PrefixList")) {
          inclusiveNamespaceElements[k].getAttribute("PrefixList").split(/\s+/).forEach(function (ns) {
            inclusiveNamespaces.push(ns);
          });
        }
      }
      transforms.push({
        algorithm: transformElements[j].getAttribute("Algorithm"),
    ...
    var transformed = transforms.reduce(function(node, transform) {
      return this.transforms[transform.algorithm].transform(node, transform.inclusiveNamespaces);
    }.bind(this), target.cloneNode(true));
